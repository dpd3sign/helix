...
export default function GroceryListModal() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Export queued",
        "Your grocery list will be delivered to the configured integration.",
      );
    } catch (error) {
      Alert.alert(
        "Export failed",
    ...
    <ThemedView style={styles.container}>
      ...
      >
        <ThemedText style={styles.copy}>
          Export hooks will send this list to Instacart or similar. Toggle
          purchased or auto-synced via Supabase updates.
        </ThemedText>
        <View
        ...
        >
          {items.map((item) => (
            <View key={item.name} style={styles.listRow}>
              ...
              <ThemedText style={styles.listQuantity}>
                {item.quantity}
              </ThemedText>
            </View>
          ))}
