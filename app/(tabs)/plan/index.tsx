...
export default function PlanOverviewScreen() {
  ...
  const regeneratePlan = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const { error } = await supabase.functions.invoke('generate-weekly-plan', {
        body: { user_id: session.user.id },
      });
      if (error) throw error;
      Alert.alert('Plan regeneration requested', 'We will notify you when the adaptive plan is ready.');
      fetchPlan();
    } catch (error) {
      Alert.alert('Unable to regenerate', (error as Error).message ?? 'Please try again.');
      setLoading(false);
    }
  ...
  const todayMeals = useMemo(() => {
    ...
    return plan.meal_plans.find((item) => item.date === today) ?? plan.meal_plans[0] ?? null;
  }, [plan?.meal_plans]);

  const quickActions = useMemo(() => {
    const params = plan ? { planId: plan.plan_id } : undefined;
    return [
      { icon: 'sparkles-outline' as const, label: 'Regenerate Plan', onPress: regeneratePlan },
      { icon: 'swap-horizontal-outline' as const, label: 'Swap Exercise', href: { pathname: '/(tabs)/plan/swap-exercise', params } },
      { icon: 'restaurant-outline' as const, label: 'Swap Recipe', href: { pathname: '/(tabs)/plan/swap-recipe', params } },
      { icon: 'cart-outline' as const, label: 'Grocery List', href: { pathname: '/(tabs)/plan/grocery-list', params } },
    ];
  }, [plan, regeneratePlan]);
  ...
  return (
    <ThemedView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={[styles.selector, { backgroundColor: palette.surface, borderColor: palette.borderMuted }]}>
          <ThemedText type="subtitle">Plan Overview</ThemedText>
          {!plan && !loading ? (
            <ThemedText style={styles.copy}>
              No adaptive plan found. Regenerate to seed your first cycle once biometrics are available.
            </ThemedText>
          ) : (
            <>
              <ThemedText style={styles.copy}>
              ...
              <View style={styles.metaRow}>
                <View style={styles.metaCard}>
                  ...
                  <ThemedText style={styles.metaValue}>{plan?.readiness_score ?? 'Pending'}</ThemedText>
                </View>
                <View style={styles.metaCard}>
        ...
        <View style={[styles.card, { backgroundColor: palette.surface, borderColor: palette.borderMuted }]}>
          {nextSessions.length === 0 ? (
            <ThemedText style={styles.placeholder}>No sessions generated yet.</ThemedText>
          ) : (
            nextSessions.map((item) => (
              <View key={item.session_id} style={styles.cardRow}>
                ...
                <ThemedText style={styles.cardStatus}>{item.status ?? 'Scheduled'}</ThemedText>
              </View>
            ))
          )}
          <Link
            href={{ pathname: '/(tabs)/plan/workout', params: { planId: plan?.plan_id ?? '' } }}
            asChild
          >
        ...
        <View style={[styles.card, { backgroundColor: palette.surface, borderColor: palette.borderMuted }]}>
          {!todayMeals ? (
            <ThemedText style={styles.placeholder}>Meal plan pending.</ThemedText>
          ) : (
            <>
              <ThemedText type="defaultSemiBold">
                {new Date(todayMeals.date).toLocaleDateString(undefined, { weekday: 'long', month: 'short', day: 'numeric' })}
              </ThemedText>
              <ThemedText style={styles.cardDetail}>
                Target: {todayMeals.target_calories ?? '—'} kcal · P {todayMeals.protein_g ?? '—'}g · C {todayMeals.carbs_g ?? '—'}g · F {todayMeals.fat_g ?? '—'}g
              </ThemedText>
              {(todayMeals.meals ?? []).map((meal) => (
                <View key={meal.meal_id} style={styles.mealRow}>
                  ...
                  <ThemedText style={styles.mealDetail}>
                    {meal.planned_calories ?? '—'} kcal · P {meal.protein_g ?? '—'}g
                  </ThemedText>
                </View>
              ))}
            </>
          ...
          <Link
            href={{ pathname: '/(tabs)/plan/meal', params: { planId: plan?.plan_id ?? '' } }}
            asChild
          >
            <Pressable style={styles.viewButton} disabled={!plan}>
            ...
            if ('href' in action && action.href) {
              return (
                <Link key={action.label} href={action.href} asChild>
                  <Pressable
                    style={[styles.actionCard, { backgroundColor: palette.surface, borderColor: palette.borderMuted }]}
                    disabled={!plan}
                  >
