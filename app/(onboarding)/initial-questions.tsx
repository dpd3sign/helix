...
const activityLevels: {
  label: string;
  value: "sedentary" | "light" | "moderate" | "high";
}[] = [
...
export default function InitialQuestionsScreen() {
  ...
  const handleNext = () => {
    const heightMetric = unitSystem === "metric"
      ? parseNumber(heightCmInput)
      : Number.NaN;
    const feet = unitSystem === "imperial"
      ? parseNumber(heightFeet)
      : Number.NaN;
    const inches = unitSystem === "imperial"
      ? parseNumber(heightInches)
      : Number.NaN;
    ...
    const metricHeightValue = unitSystem === "metric"
      ? heightMetric
      : (feet * 12 + inches) * 2.54;

    if (
      [metricHeightValue, weight, rhr, baselineHrv, sleep, bf].some(
        Number.isNaN,
      )
    ) {
      Alert.alert(
        "Invalid input",
        "Please ensure all numeric fields are valid.",
      );
      return;
    }
    ...
    <ThemedView style={styles.container}>
      ...
      >
        ...
        <ThemedText style={styles.copy}>
          These inputs feed your Supabase profile and biometric baseline tables.
          We’ll compute macros and readiness thresholds from here.
        </ThemedText>
          ...
          <View style={styles.chipRow}>
            {(["metric", "imperial"] as const).map((option) => (
              <Pressable
                ...
                style={[
                  styles.chip,
                  unitSystem === option && styles.chipActive,
                ]}
              >
                <ThemedText
                  style={unitSystem === option
                    ? styles.chipActiveLabel
                    : styles.chipLabel}
                >
                  {option.toUpperCase()}
          ...
          <View style={styles.chipRow}>
            {activityLevels.map((option) => (
              <Pressable
                ...
                style={[
                  styles.chip,
                  activityLevel === option.value && styles.chipActive,
                ]}
              >
                <ThemedText
                  style={activityLevel === option.value
                    ? styles.chipActiveLabel
                    : styles.chipLabel}
                >
                  {option.label}
