...
-- Run via `supabase db push` or load into your Supabase instance.
...
create index if not exists mv_body_overview_user_idx on mv_body_overview (user_id, captured_at desc);
...
create index if not exists mv_mindset_confidence_user_idx on mv_mindset_confidence (user_id, date desc);
