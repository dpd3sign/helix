...
function applyGoalAdjustment(goal: Goal, tdee: number, weeklyRate?: number): { kcalTarget: number; explanation?: string } {
  const explanations: Record<Goal, string> = {
    fat_loss: "Applied a moderate caloric deficit (~15%) to support fat loss while preserving energy.",
    muscle_gain: "Added a gentle surplus (~12%) to drive muscle gain with controlled fat accrual.",
  ...
  return { kcalTarget: Math.max(target, 1200), explanation: explanations[goal] };
}

function buildMacros(input: EPEInput, ctx: { kcalTarget: number; explanations: string[] }): DerivedContext["macros"] {
  ...
  if (primary_goal === "fat_loss" || leaner) {
    proteinPerKg = 2.2;
    ctx.explanations.push("Protein set high (≈2.2 g/kg) to preserve lean mass while leaning out.");
  } else if (primary_goal === "muscle_gain") {
    proteinPerKg = 2.0;
    ctx.explanations.push("Protein emphasized (~2.0 g/kg) to support hypertrophy and recovery.");
  }
  ...
  if (fatCalories / ctx.kcalTarget < 0.2) {
    ctx.explanations.push("Fats held at ≥20% of calories to maintain hormone health.");
  }

  let remainingCalories = ctx.kcalTarget - (proteinCalories + fatCalories);
  if (primary_goal === "endurance") {
    remainingCalories = Math.round(ctx.kcalTarget * 0.55 - proteinCalories - fatCalories);
    ctx.explanations.push("Carbohydrates biased upward to fuel endurance blocks.");
  }
...
function determineReadiness(input: EPEInput): number {
  const base = 75;
  const deductions = [];
  if (input.recent_metrics?.hrv_rmssd && input.recent_metrics.hrv_rmssd < 50) deductions.push(10);
  if (input.recent_metrics?.sleep_score && input.recent_metrics.sleep_score < 70) deductions.push(8);
  if (input.stress_baseline && input.stress_baseline >= 4) deductions.push(5);
  if (input.motivation_baseline && input.motivation_baseline <= 2) deductions.push(5);
  return Math.max(40, Math.min(95, base - deductions.reduce((a, b) => a + b, 0)));
}

function getTagArray(tags: RecipeTags | ExerciseTags | undefined, key: string): string[] {
  if (!tags) return [];
...
function matchesDiet(recipe: RecipeRow, diet: DietType): boolean {
  ...
  if (diet === "vegetarian") return recipe.diet_type === "vegetarian" || recipe.diet_type === "vegan";
  if (diet === "vegan") return recipe.diet_type === "vegan";
  if (diet === "pescatarian") return recipe.diet_type === "pescatarian" || recipe.diet_type === "omnivore";
  if (diet === "paleo") {
    const diets = getTagArray(recipe.tags, "diet");
    return diets.includes("paleo") || recipe.diet_type === "omnivore";
  }
  if (diet === "keto") {
    return recipe.tags?.macro_focus === "low_carb" || recipe.tags?.macro_focus === "balanced";
  }
...
function filterRecipes(input: EPEInput, recipes: RecipeRow[]): RecipeRow[] {
  return recipes
    ...
    .filter((recipe) => recipe.allergens.every((a) => !input.allergies.includes(a)))
    .filter((recipe) => input.restricted_foods.every((restricted) =>
      JSON.stringify(recipe.ingredients).toLowerCase().includes(restricted.toLowerCase()) === false
    ));
}
...
function buildWeekPlan(input: EPEInput, ctx: DerivedContext, recipes: RecipeRow[], exercises: ExerciseRow[]): WeekPlan {
  const week: DayPlan[] = [];
  ...
  if (recipePool.length < 4) {
    ctx.explanations.push("Limited recipe matches; rotating available meals while respecting dietary filters.");
  }
  if (exercisePool.length < 5) {
    ctx.explanations.push("Exercise pool constrained by equipment; reusing movements to maintain consistency.");
  }
...
function deriveContext(input: EPEInput): DerivedContext {
  ...
  const goalAdjustment = applyGoalAdjustment(input.primary_goal, tdee, input.weekly_change_rate);
  if (goalAdjustment.explanation) explanations.push(goalAdjustment.explanation);

  if (age >= 50) {
    explanations.push("Training volume dialed back ~10% to match recovery trends after age 50.");
  }
  if (input.recent_metrics?.steps && input.recent_metrics.steps < 6000) {
    explanations.push("Daily step target increased (6-8k) to boost NEAT and recovery quality.");
  }
  if (input.recent_metrics?.hrv_rmssd && input.recent_metrics.hrv_rmssd < 45) {
    explanations.push("Low HRV detected; recovery days scheduled to avoid overreaching.");
  }
  ...
  if (input.training_age === "new") {
    explanations.push("Progression starts with controlled tempos and foundational movements for new trainees.");
  }
...
async function fetchRecipePool(client: SupabaseClient, input: EPEInput): Promise<RecipeRow[]> {
  const { data, error } = await client
    .from("recipes")
    .select("id, name, kcal, protein_g, carbs_g, fat_g, diet_type, allergens, ingredients, tags");
  if (error) throw error;
  return data as RecipeRow[];
}

async function fetchExercisePool(client: SupabaseClient): Promise<ExerciseRow[]> {
  const { data, error } = await client.from("exercises").select("id, name, description, tags");
  if (error) throw error;
...
async function persistPlan(
  ...
  const { data: insertedPlan, error: planError } = await client.from("plans").insert({
  ...
  if (workoutRows.length > 0) {
    const { error: workoutError } = await client.from("workouts").insert(workoutRows);
...
function validateInput(input: EPEInput): string[] {
  ...
  if (!input.height_cm || input.height_cm < 120) errors.push("height_cm unrealistic");
  if (!input.weight_kg || input.weight_kg < 40) errors.push("weight_kg unrealistic");
  if (!input.goal_duration_weeks) errors.push("goal_duration_weeks is required");
  if (!input.training_days_per_week) errors.push("training_days_per_week is required");
