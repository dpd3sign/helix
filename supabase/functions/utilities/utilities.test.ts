...
async function fetchMealByKeys(
  ...
  let q = client
    .from("meals")
    .select(
      // NOTE: no created_at — some schemas don't have it
      "id, plan_day_id, name, meal_type, kcal, protein_g, carbs_g, fat_g, recipe_id",
    );
  ...
  // If we have no plan_day_id filter, pick a stable latest by id
  if (!planDayId) q = q.order("id", { ascending: false });
...
export async function handleSwapRecipe(
  ...
  if (to_recipe_id !== null) {
    updatePayload.recipe_id = to_recipe_id; // FK enforces validity
  }
